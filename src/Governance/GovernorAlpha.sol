...
contract GovernorAlpha {
    ...
    function propose(
        ...
        require(
            targets.length <= proposalMaxOperations(), "GovernorAlpha::propose: too many actions"
        );
    ...
    function cancel(uint256 proposalId) public {
        ProposalState states = state(proposalId);
        require(
            states != ProposalState.Executed,
    ...
    function __queueSetTimelockPendingAdmin(address newPendingAdmin, uint256 eta) public {
        ...
        timelock.queueTransaction(
            address(timelock), 0, "setPendingAdmin(address)", abi.encode(newPendingAdmin), eta
        );
    }
